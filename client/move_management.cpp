...
bool ControlMove::checkAlignment(shared_ptr<Case> case_main, shared_ptr<Case> neighborsPawnCase, shared_ptr<Case> neighborsWallCase) {
    ...
    if(neiNeighborsPos.x == case_main->getPos().x || neiNeighborsPos.y == case_main->getPos().y){
        vector<shared_ptr<Case>> wall_neigh = case_main->getWallCaseNeighbors();
...
bool ControlMove::isAligner(shared_ptr<Case> case1, shared_ptr<Case> case2, shared_ptr<Case> wall) {
...
bool ControlMove::isWallBetween(shared_ptr<Case> pawnCase1, shared_ptr<Case> pawnCase2, shared_ptr<Case> wallCase) {
    vector<shared_ptr<Case>> wallCase1 = pawnCase1->getWallCaseNeighbors();
    vector<shared_ptr<Case>> wallCase2 = pawnCase2->getWallCaseNeighbors();
...
void ControlMove::printPos(vector<shared_ptr<Case>> vec) {
...
vector<shared_ptr<Case>>  ControlMove::checkMovement(shared_ptr<Case> case_main) {
    // Est
    vector<shared_ptr<Case>> res;
    vector<shared_ptr<Case>> neighborsWallCase = case_main->getWallCaseNeighbors();
    vector<shared_ptr<Case>> neighborsPawnCase = case_main->getPawnCaseNeighbors();
    ...
    for (size_t i = 0; i  < neighborsPawnCase.size(); ++i) {
        ...
        bool isCase1 = false;
        vector<shared_ptr<Case>> neighborsPawnCaseOfNeighbor = neighborsPawnCase[i]->getPawnCaseNeighbors();
        vector<shared_ptr<Case>> neighborsWallCaseOfNeighbor = neighborsPawnCase[i]->getWallCaseNeighbors();
        for (size_t j=0; j < neighborsPawnCaseOfNeighbor.size(); j++) {
            ...
            shared_ptr<Case> wall = neighborsWallCaseOfNeighbor[j];
        ...
        for (size_t j=0; j < neighborsPawnCaseOfNeighbor.size(); j++) {
            shared_ptr<Case> possiblePawnCase = neighborsPawnCaseOfNeighbor[j];
            shared_ptr<Case> possibleWallCase = neighborsWallCaseOfNeighbor[j];
