...
void gameWindow::make_buttons(){
    ...
    connect(mapper, SIGNAL(mappedString(QString)), this, SLOT(play_game(QString)));
    for(int i = 0; i<17; i++){
        buttons.push_back({});
...
void gameWindow::updateButtons() const{
...
void gameWindow::delete_buttons(){
    // delete les buttons
    for(int i = 0; i < 17; i++){
        for(int j = 0; j < 17; j++){
            delete buttons[i][j];
...
bool gameWindow::isCasePawn(std::string move){
